...
const styles = StyleSheet.create({
  ...
  barWrap: { height: 180, alignItems: 'flex-end', paddingVertical: 8, flexDirection: 'row', justifyContent: 'space-between' },
...
export default function WeeklyStats({ route }: Props) {
  ...
  // podľa typu
  const byType = weekRecs.reduce<Record<string, { sessions: number; minutes: number }>>((acc, r) => {
    const t = inferType(r);
                ...
                {typeRows.map(r => (
                  <View key={`bar-${r.type}`} style={styles.hrBarRow}>
                    ...
                    <View style={styles.hrBarTrack}>
                      <View style={[styles.hrBarFill, { width: `${(r.minutes / maxTypeMin) * 100}%` }]} />
                    </View>
                    <Text style={{ width: 48, textAlign: 'right', marginLeft: 8 }}>{r.minutes}</Text>
                  </View>
                ))}
              </View>
            </>
